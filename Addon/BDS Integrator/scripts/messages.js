...
export const messages = {
    ...
    status: (sender) => {
        const message = `Server connected to the BDS Integration database.\n§7Chat: §a${variables.get("enable-chat")}§r\n§7Voice: §a${variables.get("enable-voice")}§r`;
        const linked = sender.hasTag('linked');
        const linkDisplay = linked ? `§atrue§r` : `§4false§r`;
        if (variables.get("enable-voice") === true) message+=`\n§8Horizontal Range: §a${variables.get("horizontal-range")}\n§8Vertical Range: §a${variables.get("vertical-range")}\n§8Player Addition: ${variables.get("player-addition")}§r`;
