        ...
        switch (command) {
            ...
            case `link`:
                if (event.sender.hasTag('linked')) {
                    event.sender.runCommand(`tellraw @s {"rawtext":[{"text":"You've already linked your Minecraft account with discord!"}]}`);
                } else {
                    ...
                    event.sender.runCommand(`tellraw @s {"rawtext":[{"text":"Use §d/auth§r in DMs with the BDS Integration bot using code §a${code}§r to link your Minecraft account with Discord."}]}`);
                }
                break;
            case `unlink`:
                if (event.sender.hasTag('linked')) {
